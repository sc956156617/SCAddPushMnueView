...
@interface SCAddPushMnueView () <UITableViewDataSource, UITableViewDelegate>
...
@property (nonatomic, assign) SCPushMnueViewDirection direction;

@end

@implementation SCAddPushMnueView
...
- (instancetype)initWithDataArray:(NSArray *)dataArray
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)]) {
        ...
        if (direction == kSCPushMnueViewDirectionLeft) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //取出模型
    SCCellModel *model = [self.dataArray objectAtIndex:indexPath.row];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.direction == kSCPushMnueViewDirectionLeft) {
...
#pragma mark - SCCellModel

@implementation SCCellModel

- (instancetype)initWithTitle:(NSString *)title imageName:(NSString *)imageName {
    SCCellModel *model = [[SCCellModel alloc] init];
    model.title = title;
